...
DATA:               ls_service_os          TYPE LINE OF crmt_srv_osset_comt,
                    ...
                    gt_service_os          TYPE crmt_srv_osset_comt.
...
PARAMETERS          schemaid                TYPE string OBLIGATORY DEFAULT 'YBP_SERVICE'.
...
START-OF-SELECTION.
  ...
  DATA(lt_schema) = new ZCL_HOME_SCHEMA_TOOL( )->get_hierarchy_data( schemaid ).
  DATA(lv_line) = lines( lt_schema ).
  ASSERT lv_line > 0.

  DATA(go_schema) = cl_abap_random_int=>create( min = 1 max = lv_line ).
...
FORM prepare_service_os USING iv_header_guid TYPE crmt_object_guid.
  CLEAR: ls_service_os, ls_osset, ls_subject, lt_osset.
  ls_service_os = VALUE #( ref_guid = iv_header_guid ref_kind = 'A' ).
  ...
  ls_subject-asp_id = schemaid.
  ls_subject-katalog_type = 'D'.
  data(lv_schema_index) = go_schema->get_next( ).

  ls_subject-cat_id = lt_schema[ lv_schema_index ]-id.
  ls_subject-mode = 'A'.
  ...
  ls_input_field-ref_guid = lv_header_guid.
  ...
  APPEND ls_input_field_names TO ls_input_field-field_names.
  APPEND ls_input_field TO gt_input_fields.
...
FORM handle_order_with_number USING iv_num TYPE int4.
  ...
  gt_schedlin_i_com, gt_obj_guids, gt_input_fields, gt_service_os.
  DO iv_num TIMES.
    lv_char10 = sy-index.
    ...
    IF p_type = 'SRVR'.
       PERFORM prepare_service_os using lv_header_guid.
    ENDIF.
...
*  lo_price->cleanup_buffer( ).
  CLEAR: cl_crms4_product_api=>gt_product_buffer, ct_header_guid.
ENDFORM.
